...
# TL;DR: daemon to watch a folder and execute any scripts that appear in that folder.
#
...
daemon() {
    ...
    do
        ...
        #This was the original implementation, which doesn't always open a separate terminal
        #nice find $1/  -name \*.sh -perm /u+x -type f -mmin $2 -exec bash -c 'myrun "$0"' {} \;

        #This implementation always opens a separate terminal to run the commands in.
        nice find $1/  -name \*.sh -perm /u+x -type f -mmin $2 -exec gnome-terminal -- /bin/bash -c '$0' {} \;
        sleep $2m
    done
}
