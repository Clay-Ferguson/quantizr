...
public class NodeEditService extends ServiceBase {
    ...
    public CreateSubNodeResponse createSubNode(MongoSession ms, CreateSubNodeRequest req) {
        ...
        if (allowSharing) {
            ...
            if (req.getShareToUserId() != null) {
                ...
                if (
                    nodeBeingRepliedTo.getBool(NodeProp.UNPUBLISHED) && nodeBeingRepliedTo.getOwner().equals(ms.getUserNodeId())
                ) {
        ...
        res.setNewNode(
            convert.convertToNodeInfo(
                ...
                newNode,
                false, //
                req.isCreateAtTop() ? 0 : Convert.LOGICAL_ORDINAL_GENERATE,
                false,
                false,
                false,
                false,
    ...
    public SaveNodeResponse saveNode(MongoSession ms, SaveNodeRequest req) {
        ...
        NodeInfo newNodeInfo = convert.convertToNodeInfo(
            false,
            ThreadLocals.getSC(),
            ms,
            node,
