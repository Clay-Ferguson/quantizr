...
public class UserManagerService extends ServiceBase {
    ...
    public void processLogin(
        ...
        if (!StringUtils.isEmpty(nostrNpub)) userNode.setIfNotExist(NodeProp.NOSTR_USER_NPUB, nostrNpub);
    ...
    public SaveUserPreferencesResponse saveUserPreferences(SaveUserPreferencesRequest req) {
        ...
        arun.run(as -> {
            ...
            if (prefsNode == null) throw new RuntimeException("Unable to update preferences.");
    ...
    public void writeProfileToIPNS(SessionContext sc, String userName, String bio, String displayName) {
        ...
        exec.run(() -> {
            arun.run(as -> {
                ...
                if (pathStat == null) {
                    push.sendServerPushInfo(sc, new PushPageMessage("Decentralized Identity Publish FAILED", true, "note"));
                    return null;
                }
                ...
                update.save(as, userNode);
                push.sendServerPushInfo(sc, new PushPageMessage("Decentralized Identity Publish Complete.", false, "note"));
                return null;
            });
        });
    }
    ...
    public BlockUserResponse blockUser(MongoSession ms, BlockUserRequest req) {
        ...
        if (userNode == null) {
            ...
            if (accntNode == null) throw new RuntimeException("User not found.");
        ...
        arun.run(as -> {
            ...
            if (node.getAc() != null) {
                ...
                for (String accntId : node.getAc().keySet()) {
                    ...
                    if (
                        accntId.equals(ownerIdStr) || idSet.contains(accntId) || PrincipalName.PUBLIC.s().equals(accntId)
                    ) continue;
