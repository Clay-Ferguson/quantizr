...
public class MongoDelete {
	...
	/*
	 * When a user is creating a new node we leave FIELD_MODIFY_TIME null until
	 * their first save of it and during the time it's null no other users can see
	 * the node. However the user can also abandon the browser or cancel the editing
	 * and orphan the node that way, and this method which we call only at startup,
	 * cleans up any and all of the orphans
	 */
	...
	public void cleanupOldTempNodesForUser(MongoSession session, SubNode userNode) {
		...
		Criteria criteria = Criteria.where(SubNode.FIELD_PATH)
				.regex(util.regexRecursiveChildrenOfPath(userNode.getPath())) //
		...
		if (res.getDeletedCount() > 0) {
			log.debug("Temp Records Deleted (Under User: " + userNode.getId().toHexString() + "): "
					+ res.getDeletedCount());
		}
	}
	 ...
	 * so when we delete one ore more nodes, potentially orphaning other nodes or
	 ...
	 * 
	 * UPDATE: In here we call "regexRecursiveChildrenOfPath" which does the most
	 * aggressive possible delete of the entire subgraph, but it would be more
	 * performant to use a non-recursive delete ONLY of the direct children under
	 * 'node' (using "regexDirectChildrenOfPath" instead) in this synchronous call
	 * and let the orphan delete do the rest of the cleanup later on asynchronously.
	 * However we do NOT do that, only because our orphan cleanup algo is already
	 * memory intensive and so we are decreasing the memory load of the orphan
	 * cleanup routing by doing the most aggressive tree delete possible here,
	 * synchronously, at he cost of a little performance.
	 * 
	 * Said more simply: Replace 'regexRecursiveChildrenOfPath' with
	 * 'regexDirectChildrenOfPath' in this method if you want to increase
	 * performance of deletes at the cost of some additional memory.
	 */
	...
	/*
	 * This algorithm requires one hash value of memory for every non-leaf node in
	 * the DB to run so it's very fast but at the cost of memory use
	 */
	public void deleteNodeOrphans(MongoSession session) {
		...
		for (SubNode node : nodes) {
			 ...
			 * be super paranoid we just recalculate that hash here since the consequences
			 * of it being out of sync could be data loss.
			 */
		...
		/*
		 * Run this up to 10 per call. Note that for large orphaned subgraphs we only
		 * end up pruning off the 10 deepest levels at a time, so running this multiple
		 * times will be required, but this is ideal. We could raise this 10 to a larger
		 * number larger than any possible tree depth, but there's no need. Running this
		 * several times has the same effect
		 */
		while (++loops < 10) {
			/*
			...
			for (SubNode node : nodes) {
				...
				if (!pathHashSet.contains(DigestUtils.sha256Hex(node.getParentPath()))) {
					 ...
					 * Theoretically the node.getPathHash() should already contain this hash, but to
					 * be super paranoid we just recalculate that hash here since the consequences
					 * of it being out of sync could be data loss. Also I do realize we alrady
					 * calculated this hash above and could have stored it in memory to use here,
					 * but we already have pathHashSet being a large memory use here, so we opt to
					 * burn more CPU cycles for this second hash calc than to burn the memory by
					 * storing it above.
					 */
